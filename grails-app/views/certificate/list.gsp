			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="cerType" title="${message(code: 'certificate.cerType.label', default: 'Cre Type')}" />
					
						<g:sortableColumn property="cerTitle" title="${message(code: 'certificate.cerTitle.label', default: 'Cer Title')}" />
				...
				<g:each in="${certificateInstanceList}" status="i" var="certificateInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${certificateInstance.id}">${fieldValue(bean: certificateInstance, field: "cerType")}</g:link></td>
